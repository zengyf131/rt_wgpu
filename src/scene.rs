...
pub fn final_scene() -> (Camera, Box<dyn Primitive>) {
    ...
    let light = Rc::new(RefCell::new(DiffuseLight::from_color(vec3(7.0, 7.0, 7.0))));
    world.add(Box::new(Quad::new(vec3(123.0,554.0,147.0), vec3(300.0,0.0,0.0), vec3(0.0,0.0,265.0), light)));
    ...
    world.add(Box::new(Sphere::sphere_moving(center1, center2, 50.0, sphere_material)));

    world.add(Box::new(Sphere::sphere(vec3(260.0, 150.0, 45.0), 50.0, Rc::new(RefCell::new(Dielectric::new(1.5))))));
    world.add(Box::new(Sphere::sphere(
        vec3(0.0, 150.0, 145.0), 50.0, Rc::new(RefCell::new(Metal::new(vec3(0.8, 0.8, 0.9), 1.0)
    )))));

    let boundary = Box::new(Sphere::sphere(vec3(360.0,150.0,145.0), 70.0, Rc::new(RefCell::new(Dielectric::new(1.5)))));
    world.add(boundary);
    let boundary = Box::new(Sphere::sphere(vec3(360.0,150.0,145.0), 70.0, Rc::new(RefCell::new(Dielectric::new(1.5)))));
    world.add(Box::new(ConstantMedium::from_color(boundary, 0.2, vec3(0.2, 0.4, 0.9))));
    let boundary = Box::new(Sphere::sphere(vec3(0.0,0.0,0.0), 5000.0, Rc::new(RefCell::new(Dielectric::new(1.5)))));
    world.add(Box::new(ConstantMedium::from_color(boundary, 0.0001, vec3(1.0,1.0,1.0))));

    let image_bytes = include_bytes!("earthmap.jpg");
    let emat = Rc::new(RefCell::new(Lambertian::new(Rc::new(RefCell::new(ImageTexture::from_bytes(image_bytes))))));
    world.add(Box::new(Sphere::sphere(vec3(400.0,200.0,400.0), 100.0, emat)));
    let pertext = Rc::new(RefCell::new(NoiseTexture::new(0.2)));
    world.add(Box::new(Sphere::sphere(vec3(220.0,280.0,300.0), 80.0, Rc::new(RefCell::new(Lambertian::new(pertext))))));

    let mut boxes2 = PrimitiveList::new();
    let white = Rc::new(RefCell::new(Lambertian::from_color(vec3(0.73, 0.73, 0.73))));
    let ns: usize = 1000;
    for j in 0..ns {
        boxes2.add(Box::new(Sphere::sphere(vec3(rng.random_range(0.0..165.0), rng.random_range(0.0..165.0), rng.random_range(0.0..165.0)), 10.0, white.clone())));
    }

    world.add(Box::new(Translate::new(
        Box::new(RotateY::new(
            Box::new(BVHNode::from_prim_list(boxes2)), degrees(15.0))),
